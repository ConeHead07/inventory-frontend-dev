...
export class DBSyncClientService implements OnDestroy {
  ...
  async askServerForChanges(jobid: number = 0): Promise<ServerChangesStatusInfo> {
    ...
    if (!this.networkService.getCurrentState().hasValidSession) {
      console.error('Abort Change-Request: No valid Session!');
      return { success: false, errorMsg: 'Abort Change-Request: No valid Session!', MaxRevisionId: -1, NumChanges: -1 };
    }

    console.log('Start Ask for Change-Request');
  ...
  async sendByJobId(useJobid: number, useLogs?: DBDIClientChangeLog[], useJobResult?: SyncJobResult): Promise<SyncJobResult> {
    ...
    if (!this.networkService.getCurrentState().hasValidSession) {
      return this.finishProcess(
        syncJobResult,
        SyncJobStatus.Offline,
        'Synchronisatioon wurde abgebrochen. No valid Session!'
      );
    }
